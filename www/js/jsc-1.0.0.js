...
//customIRCELINE
  info: {
    ...
    head_PM10: 'PM<sub>10</sub>',
    head_PM25: 'PM<sub>2.5</sub>',
    index_1: 'ausgezeichnet',
    index_2: 'sehr gut',
    index_3: 'gut',
    index_4: 'relatief gut',
    index_5: 'mäßig',
    index_6: 'ungenügend',
    index_7: 'sehr ungenügend',
    index_8: 'schlecht',
    ...
    O3: 'Ozon',
    PM25: 'Feinstaub < 2.5 µm',
    PM10:'Feinstaub < 10 µm',
    BC_description: 'Schwarzer Kohlenstoff (BC) oder Ruß ist der am stärksten lichtabsorbierende Bestandteil vom Feinstaub (PM) und wird während der unvollständigen Verbrennung fossiler Energieträger (z.B. Diesel), Biokraftstoffe (z.B. Biodiesel) und Biomasse (z.B. Holz) erzeugt. BC wird direkt in die Atmosphäre in Form von feinen Partikeln (kleiner als 2,5 µm, sehe Beschreibung PM<sub>2.5</sub>) emittiert. Die direkte Toxizität von BC ist noch nicht vollständig geklärt, aber es ist deutlich, dass dieser Schadstoff als Träger für andere Schadstoffe dient. Es besteht ein Verband zwischen der Aussetzung an Feinstaub und dem Risiko der Entwicklung von Krebs. Emissionen von Dieselmotoren wurden durch die Weltgesundheitsorganisation (WHO) als krebserzeugend klassifiziert. Dieselemissionen sind ein wichtiger Teil des Black Carbons, das andere stark krebserregende Stoffe, wie beispielsweise polycyclische Kohlenwasserstoffe absorbieren kann. BC ist auch bekannt als \"kurzlebiger klimawirksamer Schadstoff\". Dies bedeutet, dass es einen klaren Zusammenhang zwischen Klimawandel und BC gibt,  das dieser Schadstoff aber viel kürzer als z.B. Kohlendioxid (CO<sub>2</sub>) in der Atmosphäre bleibt. Nach CO<sub>2</sub> ist BC der zweitwichtigste Faktor im Bezug auf den Klimawandel.',
    NO2_description: 'Stickstoffdioxid (NO<sub>2</sub>) wird bei der Verbrennung bei hohen Temperaturen (z.B. Diesel, Industrieprozessen) emittiert. NO<sub>2</sub> ist ein guter Proxy für die Verkehrsbedingte Luftverschmutzung, aber es gibt auch bedeutende Industriequellen. NO<sub>2</sub> ist ein wichtiger Präkursor für die Bildung von Ozon (Sommer-Smog) und "sekundäre anorganische Aerosolen" (feine Partikel - Feinstaub). Es gibt immer mehr Hinweise der nachteiligen Auswirkungen auf die Gesundheit dieses Schadstoffs (Atemnot, Entzündung der Atemwege).',
    O3_description: 'Hohe Ozon (O<sub>3</sub>) Konzentrationen assoziiert man durchgehend mit heißen, sonnigen Tagen. O<sub>3</sub> wird in einer photochemischen Reaktion gebildet, wenn Stickoxide (NOx) und flüchtige organische Verbindungen (VOCs) in der Atmosphäre reagieren in der Gegenwart von Sonnenlicht (UV - Ultraviolettstrahlung). Ozonkonzentrationen sind in der Regel niedriger in der Nähe von Verkehrsemissionen, da Dieselmotoren auch kurtzlebiges Stickstoffmonoxid (NO) emittieren, das das gebildete O<sub>3</sub> konsumiert. Innenozonkonzentrationen sind typischerweise etwa halb so hoch wie im Freien während Sommersmogepisoden. Hohe Ozonkonzentration haben deutlich negative Nebenwirkungen auf die Atemwege.',
    PM10_description: 'Die Masse aller Teilchen, die kleiner als 10 µm sind. Diese Fraktion vom Feinstaub hat viele andere Quellen neben Verbrennung und die wissenschaftlichen Beweise für nachteilige Auswirkungen auf die Gesundheit sind gut dokumentiert.',
...
//customIRCELINE
  info: {
    ...
    BC_description: 'Le Black carbon (BC) ou carbone-suie est la principale espèce particulaire absorbante de l’air. Il se forme lors de la combustion incomplète des carburants fossiles (par exemple diesel), des bio carburants (par ex. bio-diesel) ou de la biomasse (par ex. le bois). BC est émis directement dans l’atmosphère sous la forme de particules fines (inférieures à 2,5 µm, cf. PM<sub>2.5</sub>). La toxicité directe du BC n’est pas clairement établie mais sa capacité d’agir comme vecteur pour d’autres polluants est avérée. Il existe un lien entre l’exposition aux particules fines et le risque de développement de cancer. Les émissions des moteurs diesel sont classées comme cancérogènes par l’OMS, ces émissions contiennent une part importante de black carbon auquel peuvent être adsorbés d’autres polluants tels que des hydrocarbures polycycliques. Par exemple. BC est également connu pour être un polluant climatique de courte durée. Ce qui signifie que le BC a un impact direct sur le réchauffement climatique. mais que ce composé a une durée de vie dans l’atmosphère beaucoup plus courte que le dioxyde de carbone (CO<sub>2</sub>) par exemple. Après le CO<sub>2</sub>, le BC est le second plus important composé influençant le changement climatique.',
    NO2_description: 'Le dioxyde d’azote est émis durant les processus de combustion à haute température (par exemple les  moteurs diesel, les procédés industriels).  NO<sub>2</sub> est considéré comme un bon indicateur de la pollution de l’air par le trafic, mais il existe également des sources industrielles importantes. NO<sub>2</sub> est un important précurseur pour la formation d’ozone (SMOG estival) et des “aérosols secondaires inorganiques” (particules fines). Il y a de plus en plus d’indications qui mettent en évidence les effets néfastes de ce polluant sur la santé (altération des fonctions respiratoires, inflammations des voies respiratoires).',
    O3_description: 'Des concentrations élevées d’Ozone (O<sub>3</sub>) sont généralement observes lors des journées chaudes et ensoleillées. O<sub>3</sub> se forme par réaction photochimique (c’est à dire en présence du rayonnement solaire) entre divers polluants tels que les oxydes d’azotes (NOx) ou les composés organiques volatils (VOC) dans l’atmosphère. Les concentrations d’ozone sont généralement plus faibles à proximité du trafic suite aux émissions de monoxyde d’azote, composé à courte durée de vie, qui consomme une partie de l’ozone formé. Les concentrations d’ozone sont généralement deux fois plus faibles à l’intérieur des habitations durant les pics. A des concentrations trop élevées, l’ozone a des effets marqués sur la santé humaine, notamment au niveau respiratoire.',
...
//customIRCELINE
  info: {
    ...
    NO2: 'Stikstofdioxide',
    O3: 'Ozon',
    PM25: 'Fijn stof < 2.5 µm',
    PM10:'Fijn stof < 10 µm',
    BC_description: 'Zwarte koolstof (Black Carbon – BC) of roet is de sterkst lichtabsorberende component van fijnstof (PM) en wordt gevormd bij onvolledige verbranding van fossiele brandstoffen (bv. diesel), biobrandstoffen (bv. biodiesel) en biomassa (bv. hout). BC wordt rechtstreeks uitgestoten in de atmosfeer in de vorm van fijne deeltjes (kleiner dan 2,5 micrometer, zie PM<sub>2.5</sub>). De directe toxiciteit van BC is nog niet volledig uitgeklaard, maar het is wel duidelijk dat deze polluent als een drager fungeert voor andere polluenten. Er is een verband tussen de blootstelling aan fijn stof en het risico op de ontwikkeling van kanker. De emissies van dieselmotoren werd als carcinogeen geklasseerd door de WGO. Deze emissies zijn een belangrijk onderdeel van Black Carbon die andere sterk kankerverwekkende stoffen kunnen absorberen, zoals b.v. polycyclische koolwaterstoffen. BC is ook bekend als een "short-lived climate forcer". Dit betekent dat er een duidelijke relatie is tussen klimaatverandering en BC, maar dat deze verontreinigende stof veel korter in de atmosfeer blijft dan bv. koolstofdioxide (CO<sub>2</sub>). Na CO<sub>2</sub>, is BC de tweede meest belangrijke factor met betrekking tot klimaatverandering.',
